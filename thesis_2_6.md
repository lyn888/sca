...
本章围绕后续脉冲神经网络剪枝研究所需的理论基础与关键技术进行了系统梳理。首先，从脉冲神经网络的基本理论出发，分析了其生物学启发、结构特点以及与传统人工神经网络在信息表示方式、动态建模机制和计算模式上的差异，明确了 SNN 在事件驱动计算和时序信息处理方面的独特属性。随后，围绕 IF、LIF 等典型脉冲神经元模型及频率编码、时间编码等信息表达方式展开论述，进一步说明了脉冲神经网络进行时空信息处理的基础机理。

在训练与压缩方法层面，本章进一步分析了基于代理梯度的直接训练方法、时空反向传播机制及其在深层 SNN 优化中的作用，并对梯度消失、时间展开开销、脉冲活动失衡等常见训练难点进行了归纳。随后，系统梳理了非结构化剪枝、结构化剪枝以及基于重要性评估的剪枝方法，阐明了神经网络剪枝从参数删除到结构压缩、从经验式筛选到重要性建模的发展脉络，为后续面向脉冲动态特征的通道剪枝方法设计提供了理论参照。

最后，本章结合本文的研究目标，讨论了模型压缩与能耗优化之间的关系，分析了参数规模、计算复杂度、脉冲活动稀疏性与近似能耗开销之间的联系，并给出了连接保留率、紧凑模型参数量、近似 SynOps 和分类准确率等评价指标的统一口径。通过上述分析，本章完成了从脉冲神经网络基础理论、训练机制到剪枝与能耗分析框架的整体铺垫，为后续第 3 章通道重要性评估剪枝方法和第 4 章剪后恢复与重建方法的展开奠定了理论基础。
